...
router.post('/', async (req, res, next) => {
    ...
    try {
        ...
        const { rows: [bucket] } = await client.query(`INSERT INTO bucket (name, location, expiration_date, created_by)
       VALUES ($1, $2, $3, $4)
       RETURNING id, name, location, created_at, expiration_date, is_completed`, [name.trim(), location?.trim() || null, expiration_date || null, req.userId]);
        await client.query(`INSERT INTO bucket_member (bucket_id, user_id) VALUES ($1, $2)`, [bucket.id, req.userId]);
        const emails = Array.isArray(invite_emails) ? invite_emails : [];
        const invitations = [];
        for (const email of emails) {
            const { rows: [inv] } = await client.query(`INSERT INTO invitation (bucket_id, invited_email, invited_by)
         VALUES ($1, $2, $3)
         RETURNING token, invited_email`, [bucket.id, email.toLowerCase().trim(), req.userId]);
            invitations.push(inv);
        }
        await client.query('COMMIT');
        // Send emails after commit so DB state is always clean
        if (invitations.length > 0) {
            const { rows: [creator] } = await client_1.default.query(`SELECT name FROM "user" WHERE id = $1`, [req.userId]);
            for (const inv of invitations) {
                (0, email_1.sendInvitationEmail)({
